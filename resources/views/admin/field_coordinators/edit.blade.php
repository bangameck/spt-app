...
@section('title', 'Edit Field Coordinator')

@section('content')
    <div class="container-fluid">
        <div class="flex justify-between items-center mb-6">
            <h4 class="text-default-900 text-2xl font-bold">Edit Field Coordinator:
                {{ $fieldCoordinator->user->name ?? 'Tidak Ditemukan' }}</h4>
        ...
        {{-- Pesan error umum dari controller (misal dari catch image upload) --}}
        @if (session('error'))
            <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">
        ...
        <div class="card bg-white shadow rounded-lg p-6">
            <form action="{{ route('admin.field-coordinators.update', $fieldCoordinator) }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                @method('PUT') {{-- Penting: Gunakan method PUT untuk update --}}

                <h5 class="text-lg font-semibold text-default-800 mb-4">Informasi Akun Field Coordinator (Data User)</h5>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name"
                            ...
                            value="{{ old('name', $fieldCoordinator->user->name ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="username" id="username"
                            ...
                            value="{{ old('username', $fieldCoordinator->user->username ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email"
                            ...
                            value="{{ old('email', $fieldCoordinator->user->email ?? '') }}"
                            placeholder="Masukkan alamat email" required>
                        @error('email')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    {{-- Field password dan konfirmasi password (Opsional) --}}
                    <div class="relative">
                        <label for="password" class="block text-sm font-medium text-default-700 mb-2">Password Baru
                    ...
                    <div class="relative">
                        <label for="password_confirmation" class="block text-sm font-medium text-default-700 mb-2">Ulangi
                            Password Baru</label>
                ...
                <div class="mb-6">
                    <label for="img" class="block text-sm font-medium text-default-700 mb-2">Foto Profil (Kosongkan
                    ...
                    <div id="drop-area-profile"
                        ...
                        <div id="image-preview-profile" class="mt-4 flex justify-center items-center">
                            {{-- Gambar preview akan diinisialisasi oleh JS --}}
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" name="position" id="position"
                            ...
                            value="{{ old('position', $fieldCoordinator->position ?? 'Mitra Kerjasama Pengelolaan Perparkiran') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="id_card_number" id="id_card_number" maxlength="16"
                            ...
                            placeholder="Masukkan Nomor KTP (maks 16 digit)" required>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="address" id="address" rows="3"
                            ...
                            placeholder="Masukkan alamat lengkap" required>{{ old('address', $fieldCoordinator->address ?? '') }}</textarea>
                        @error('address')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label for="phone_number" class="block text-sm font-medium text-default-700 mb-2">Nomor
                            Telepon</label>
                        <input type="text" name="phone_number" id="phone_number" maxlength="14"
                            {{-- Added maxlength --}}
                            ...
                            value="{{ old('phone_number', $fieldCoordinator->phone_number ?? '') }}"
                    ...
                    <div class="md:col-span-2">
                        <label for="id_card_img" class="block text-sm font-medium text-default-700 mb-2">Foto KTP
                            (Kosongkan jika tidak ingin mengubah)</label>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            console.log('DOM Content Loaded for Field Coordinator Edit Page.');
            ...
            // `currentImageUrl` is the URL of the image currently stored for this record
            function setupImageUploader(prefix, currentImageUrl = null) {
                ...
                // Add drag/drop listeners to the drop area
                ...
                async function handleFiles(files) {
                    ...
                    imagePreview.innerHTML = ''; // Clear existing preview (old or new)

                    if (files.length === 0) {
                        fileInput.value = ''; // Ensure actual input is cleared
                        // If no new file selected, revert to old image if available
                        if (currentImageUrl) {
                            imagePreview.innerHTML =
                                `<img src="${currentImageUrl}" alt="Foto Lama" class="max-w-full h-32 object-contain rounded-md">`;
                            fileInfoDiv.classList.remove('hidden');
                            fileNameSpan.textContent = 'Foto Lama';
                            fileSizeSpan.textContent = ''; // Cannot get size easily from URL
                        }
                        return;
                    }
                    ...
                    try {
                        ...
                        if (compressedFile.size <= MAX_FILE_SIZE_BYTES) {
                            ...
                            fileInput.files = dataTransfer.files; // Set the compressed file to the input
                            ...
                            fileErrorDiv.classList.remove('hidden');
                            fileInput.value = ''; // Clear input if size is too big
                            imagePreview.innerHTML = ''; // Clear loading indicator
                        }
                    } catch (error) {
                ...
                // Initial preview for existing image on page load
                // Only show current image if no new file is selected (old() is empty for this file input)
                // and no validation error for the file itself.
                const hasOldFile = fileInput.hasAttribute('data-has-old-file') && fileInput.getAttribute(
                    'data-has-old-file') === 'true';

                if (currentImageUrl && !fileInput.files.length && !hasOldFile) {
                    imagePreview.innerHTML =
                        `<img src="${currentImageUrl}" alt="Foto Lama" class="max-w-full h-32 object-contain rounded-md">`;
                    fileInfoDiv.classList.remove('hidden');
                    fileNameSpan.textContent = 'Foto Lama';
            ...
            // Pass the current image URL from Blade to JavaScript
            const currentProfileImageUrl =
                "{{ $fieldCoordinator->user->img ? asset($fieldCoordinator->user->img) : '' }}";
            // Add a data attribute to indicate if old file exists after validation error
            const hasOldProfileFile = "{{ old('img') ? 'true' : 'false' }}";
            document.getElementById('img-upload-profile').setAttribute('data-has-old-file', hasOldProfileFile);
            setupImageUploader('profile', currentProfileImageUrl);

            // Setup for ID Card Image Uploader
            // Pass the current image URL from Blade to JavaScript
            const currentIdCardImageUrl =
                "{{ $fieldCoordinator->id_card_img ? asset($fieldCoordinator->id_card_img) : '' }}";
            // Add a data attribute to indicate if old file exists after validation error
            const hasOldIdCardFile = "{{ old('id_card_img') ? 'true' : 'false' }}";
            document.getElementById('img-upload-idcard').setAttribute('data-has-old-file', hasOldIdCardFile);
            setupImageUploader('idcard', currentIdCardImageUrl);
            ...
            if (phoneNumberInput) {
                phoneNumberInput.addEventListener('input', function(e) {
                    let value = e.target.value;
                    value = value.replace(/[^0-9]/g, '');
                    if (value.length > 14) { // New: Limit to 14 digits
