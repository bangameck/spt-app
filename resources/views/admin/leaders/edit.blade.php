...
@section('content')
    <div class="container-fluid">
        <div class="flex justify-between items-center mb-6">
            <h4 class="text-default-900 text-2xl font-bold">Edit Pimpinan: {{ $leader->user->name ?? 'Tidak Ditemukan' }}
            </h4>
        ...
        <div class="card bg-white shadow rounded-lg p-6">
            <form action="{{ route('admin.leaders.update', $leader) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label for="name" class="block text-sm font-medium text-default-700 mb-2">Nama Pimpinan
                            Lengkap</label>
                        <input type="text" name="name" id="name"
                            ...
                            value="{{ old('name', $leader->user->name ?? '') }}" placeholder="Masukkan nama lengkap"
                            required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" name="username" id="username"
                            ...
                            value="{{ old('username', $leader->user->username ?? '') }}" placeholder="Masukkan username"
                            required>
                        @error('username')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email"
                            ...
                            value="{{ old('email', $leader->user->email ?? '') }}" placeholder="Masukkan alamat email"
                            required>
                        @error('email')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="mb-6">
                    ...
                    <div id="drop-area"
                        ...
                        <div id="file-info"
                            class="mt-4 text-default-600 text-sm {{ old('img') || ($leader->user->img && !session('errors')) ? '' : 'hidden' }}">
                        ...
                        <div id="image-preview" class="mt-4 flex justify-center items-center">
                            @if ($leader->user->img && !old('img') && !session('errors'))
                                <img src="{{ asset($leader->user->img) }}" alt="Foto Profil Lama"
                                    class="max-w-full h-32 object-contain rounded-md">
                            @elseif (old('img'))
                                {{-- Jika ada error validation dan ada old img --}}
                                {{-- Placeholder for old image, actual preview generated by JS --}}
                            @endif
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label for="employee_number" class="block text-sm font-medium text-default-700 mb-2">NIP (Nomor
                            Induk Pegawai)</label>
                        <input type="text" name="employee_number" id="employee_number" maxlength="18"
                            ...
                            value="{{ old('employee_number', $leader->employee_number ?? '') }}"
                            placeholder="Masukkan NIP (maks 18 digit)" required>
                        @error('employee_number')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        <label for="start_date" class="block text-sm font-medium text-default-700 mb-2">Mulai
                            Menjabat</label>
                    ...
                    <div>
                        <label for="end_date" class="block text-sm font-medium text-default-700 mb-2">Akhir Menjabat
                            (Opsional)</label>
                        <input type="date" name="end_date" id="end_date"
                            ...
                            value="{{ old('end_date', $leader->end_date?->format('Y-m-d') ?? '') }}">
                        @error('end_date')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Fungsi untuk kompresi gambar (sama seperti di create.blade.php)
            ...
            // === START: Handle old image preview if validation fails ===
            // This is crucial for when validation fails and `old('img')` is present.
            @if (old('img') && session('errors'))
                // If there's an old image from a failed validation, try to preview it from temporary file
                // Note: Directly previewing a `File` object from `old('img')` is not straightforward
                // because `old()` returns the string path/name, not the File object.
                // A more robust solution for old image preview on validation failure
                // would involve re-uploading the temp file or saving it differently.
                // For now, we ensure the file info div is shown if there was an old attempt.
                // The user will need to re-select the file if they want to keep the new one after error.
                fileInfoDiv.classList.remove('hidden');
                fileNameSpan.textContent = 'File baru dipilih (validation failed)'; // Placeholder message
                fileSizeSpan.textContent = ''; // Can't easily get size from old()
                imagePreview.innerHTML = ''; // Clear existing preview
            @endif
            // === END: Handle old image preview ===
        });
    </script>
@endpush
